...
| 3.14 | ⏳ U TOKU — 🔴 blokirano na DB lozinci. Popis potvrdio PHP 8.2.31 (lokal 8.2.12, nizak rizik), disk 119GB/200GB slobodno. Subdomen `staging.antasline.com` kreiran, WP arhiva otpremljena i raspakovana u pravi docroot (`/home/antasline/staging/`, docroot mismatch iz FTP uploada rešen). Stalo na wp-config: DB `antasline_staging`/korisnik `antasline_antasline` postoje, lozinka nepoznata, nema je ni na serveru. #ceka-miroslav: DB lozinka (ili reset preko cPanel MySQL UI) | CC+M | jedina zavisnost bez fallbacka |
...
| M6 | 🟡 DELIMIČNO 2026-07-21 — cPanel/SSH pristup potvrđen, popis urađen, **subdomen `staging.antasline.com` kreiran** `[cpanel-live]` (docroot van public_html, SSL/DNS odmah rade preko wildcard sertifikata). Preostaje: proba migracije na subdomen (WP fajlovi+DB kopiranje, čeka M zeleno svetlo), backup raspored (WHM UI), 65 postojećih Redirection pravila odluka | migraciju (N8) | 2026-08-20 | 🔴 nema fallback — proba migracije/backup deo i dalje otvoren |

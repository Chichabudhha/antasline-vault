...
- **Dugme "Kolačići" (persistent reopen handle)** — mu-plugin `al-tracking-gtm-consent.php` je DOSLOVNA kopija LIVE koda (eksplicitno markiran "ne menjati bez M odluke" jer nosi i poznati consent-default-granted nalaz). Handle-dugme posle prihvatanja je namerna UX praksa (mogućnost da se pristanak kasnije promeni) na mnogim sajtovima, ne očigledan bug — pitanje vraćeno M-u pre izmene.

...
- 🔴🆕 **`wpGs_options` u dva sirova `mysqli` upita — pada na Linux hostingu (2026-08-14).** Nađeno prvim Copilot poslom, verifikovano nezavisnim grep-om: `migracija/alati/job-plugin-cleanup-cron.php` **linija 12** (`SELECT option_value FROM wpGs_options`) i **linija 33** (`UPDATE wpGs_options SET`). Nisu `$wpdb` nego sirovi `mysqli` — na cPanel-u gađaju nepostojeću tabelu, tiho, bez greške. Ista klasa kvara koja je oborila probu migracije 21.07 („site not installed"). Uz to šira tema: lokalni `wp-config.php` nosi `wpGs_` ([[CLAUDE]] §2), pa **~11 fajlova** sa `{$wpdb->prefix}yoast_indexable` nasleđuje pogrešan case; jedan (`job-5438-…php:257`) to već zaobilazi sa `strtolower($wpdb->prefix)`, ostali ne. **#claude-code** — popraviti dva `mysqli` upita (2 min) i odlučiti da li se `$wpdb->prefix` kaskada rešava u kodu ili u `wp-config.php`. v. [[dnevnik/2026-08-14-copilot-grok-delegati]]
- 🟢🆕 **Kvota delegata razrešena — oba su FREE (2026-08-14).** Grok: OAuth, **bez naplate** (`XAI_API_KEY` nije postavljen; log posle stvarnih poziva `subscription_tier: null` / `tier: "Free"`); `total_cost_usd` iz JSON izlaza je očitavanje brojila po API cenovniku, **ne račun**. Copilot: Free, **~50 premium zahteva mesečno ≈ 1,6 dnevno** (M potvrdio; testiranje 14.08 potrošilo ~5). 🔴 **Posledica: prvobitna premisa „delegati rasterećuju Claude kvotu" ne stoji** — oni su specijalisti za par pitanja mesečno, Claude Code ostaje radni konj. Router u `.claude/skills/delegati/SKILL.md` §1 prepisan po stvarnoj oskudnosti: `ollama` (neograničen) → `agy` → Grok → Copilot (najskuplji, **samo migraciono-kritičan pregled koda**). Nije bloker, upisano radi planiranja.
- 🟡🆕 **Grok refresh token ispisan u transkript sesije (2026-08-14).** Pri proveri tipa autentifikacije ispisan je ceo `auth.json` uključujući JWT i **refresh token** (koji ne ističe sam). **#ceka-miroslav** — rotirati: `grok logout` pa `grok login`. Usput potvrđeno da su OAuth scope-ovi bezopasni (`openid profile email offline_access grok-cli:access api:access conversations:* workspaces:*` — **ništa vezano za X nalog**) i da je `coding_data_retention_opt_out=True`.

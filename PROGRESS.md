...
- 🟡 `[claude-code]` VIZUALI — **8 Gemini promptova spremno, čeka M da generiše**: 2 za ESD stranicu (server sala, radno mesto za montažu), 3 za **„Podovi za radionice"** (najveća prilika: ~4.700 prikaza, poz. 3,5, **nema stranicu**), 2 za magacine (`podovi za skladišta` poz. 14,6 **CTR 0%**). Folder za odlaganje: `C:\Projektintasline-ai-slike\` (podfolderi po nameni). 🔴 Pravilo: AI slika **nikad** u galeriju izvedenih radova — to je lažan prodajni dokaz — [[seo/2026-08-18-ai-vizuali-promptovi]]
- ✅ `[claude-code]` W1/W2 ESD — **4 `#ceka-miroslav` pitanja zatvorena** (M dao zvanične izvore): 7 Ecotile PDF-ova u medijateku (uklj. **zapisnik ispitivanja, 7 str.**), **2 nova proizvoda** (E500/7 ESD · X-Joint antistatik ≈1×10⁹ Ω bez uzemljenja — tri ploče, katalog imao jednu), stranica 16658 dobila uporednu tabelu antistatik/ESD/elektroprovodljivo, merenje sa zapisnikom, epoksid conquest sekciju i **FAQPage schema** (nije postojala). 🔴 Stara spec tabela nosila vrednost otpora koje **nema ni u jednom tehničkom listu** — zamenjena deklarisanim. 🔴 Moj raniji nalaz „42 od 52 rečenice fale na buildu" **bio pogrešan** (build je prepisao, ne izbacio) — [[dnevnik/2026-08-18-esd-dokumentacija-i-proizvodi]]
...
- **ESD sadržaj — 4 potvrde pre objave:** deklaracija ESD serije (otpor + metod merenja) · postoji li elektroprovodljiva ≤ 10⁶ Ω (bez toga ATEX segment ne sme da se cilja) · nudimo li merenje otpora sa zapisnikom + uzemljenje · cenovni raspon. Post-live materijal, build nije diran. → [[dnevnik/2026-08-18-istrazivanje-industrijski-podovi-esd]]

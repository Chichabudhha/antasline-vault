...
**Batch 1 (prioriteti 1–5) — ✅ ZATVOREN 2026-08-07.** 3318 (ESD pod), 5276 (krovovi), 5181 (kontejneri/montažni objekti), 2622 (tri pitanja izbora poda), 3388 (štamparije) — svih 5 dobilo pravi `.al-geo-intro` "Kratak odgovor" pasus na vrhu (pre svega ostalog u `post_content`), izveden isključivo iz postojećeg teksta svakog posta. 3388 (ima FAQPage `<script>` JSON-LD) upisan preko `$wpdb->update()` (F7.24 gotcha), ostala 4 takođe preko `$wpdb->update()` radi konzistentnosti. Verifikovano: 5/5 HTTP 200, 1×H1, `.al-geo-intro` prisutan tačno 1×, JSON-LD na 3388 i dalje valid (FAQPage, 4 pitanja). Backup: `antasline-backups/antasline_local_2026-08-07_pre-w1-polish-faza4-batch1.sql`. Skripta: `migracija/alati/job-w1-polish-faza4-batch1.php`.

**Batch 2 (prioriteti 6–11) — ✅ ZATVOREN 2026-08-07 (ista sesija).** 16615 (detailing radionice), 16613 (preko starog parketa/pločica), 16612 (ftalati), 16616 (teren za pickleball), 3398 (Bergo solid/teška vozila), 2641 (PVC vs guma) — svih 6 dobilo `.al-geo-intro` pasus. 16616 ima `@graph` FAQPage+Product `<script>` JSON-LD — upisan preko `$wpdb->update()`, ostala 5 isto radi konzistentnosti. Verifikovano: 6/6 HTTP 200, 1×H1, JSON-LD na 16616 i dalje valid (`@graph`: FAQPage + Product). Skripta: `migracija/alati/job-w1-polish-faza4-batch2.php`.

**Batch 3 (prioriteti 12–21, GSC 0-klik grupa) — ✅ ZATVOREN 2026-08-07 (ista sesija) — CEO FAZA 4 SADA ZATVORENA (22/22).** 5411 (modularni podovi), 16614 (sportska igrališta/WISH kampanja), 16608 (oštećen industrijski pod), 5163 (Quectel case study), 16610 (zamena parketa sportske sale/Naxos), 3257 (ugradnja preko starih površina), 4813 (Bergo Ultimate/PLUS), 6824 (R-Tile Design supermarketi), 6874 (Secure Innovation case study), 17021 (HTEC Niš case study) — svih 10 dobilo `.al-geo-intro` pasus, isti standard (ništa izmišljeno, samo iz postojećeg teksta). Skripta: `migracija/alati/job-w1-polish-faza4-batch3.php`. Verifikovano: 10/10 HTTP 200, 1×H1, `.al-geo-intro` tačno 1× po stranici. Sitewide provera: 0 duplikata `.al-geo-intro` klase na bilo kom postu, regresija čista (homepage/industrijski-podovi/conquest 2542/3318/3388 i dalje 200).

**Napomena:** `16616` (teren-za-pickleball) više NIJE blokiran (fake-review
schema nalaz zatvoren 2026-07-28) — obrađen u batch 2 (Faza 4).

**FAZA 4 U POTPUNOSTI ZATVORENA 2026-08-07 (22/22 posta, batch 1-3, sve u istoj sesiji).**
Ceo W1 Polish red čekanja (Faze 1-4) je time zatvoren — nema poznatog preostalog
posta bez GEO-intro/CTA obrasca. Sledeći W1 rad ide kroz novi zadatak ako se pojavi.

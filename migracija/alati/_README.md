...
| **`job-w7f3-meni.php`** | **2026-07-29 — W7 F3, izvor istine za glavni meni.** Cela struktura (6 grupa × kolone × stranice) živi u nizu na vrhu skripte; meni se **ne uređuje ručno u adminu**, nego se skripta pusti ponovo. Pre upisa: proverava da svaka ciljna stranica postoji, da je `publish` i da se ne ponavlja, pa poredi sve objavljene stranice sa onim što ulazi u meni i **odbija da nastavi ako ijedna ostane bez upisanog razloga** (spisak „namerno van menija" je u skripti). Gradi **nov term** i tek onda prebacuje `main-menu` lokaciju → povratak je jedna izmena opcije. 🔴 Svaka grupa mora imati eksplicitan `_menu_item_design` — WoodMart walker nasleđuje dizajn od suseda, v. [[reference/naucene-lekcije]] |
| `job-w7f3-cene-hub.php` | 2026-07-29 — pravljenje `/cene/` hub stranice (ID 17273); primer nove stranice od nule sa Yoast metama i `_woodmart_title_off` |
| **`al_check_breadcrumbs.php`** | **2026-07-29 — provera BreadcrumbList scheme.** Za svaku ugnježdenu stranicu poredi lanac iz `post_parent`-a sa onim što se stvarno emituje u JSON-LD-u. Hvata slučaj kad Yoast **tiho ispusti pretka** jer `yoast_indexable_hierarchy.ancestor_id` pokazuje na obrisan indexable — `post_parent` je pritom potpuno ispravan, pa nijedna druga provera to ne vidi |
...
| **`job-alt-tekst-galerije.php`** | **2026-08-12 — alt tekst na slikama proizvoda (glavna + galerija), 66 priloga.** Nosi tri pravila vredna ponovne upotrebe: (1) obim se meri **po kanalu renderovanja** (`_thumbnail_id` / `_product_image_gallery` / `<img>` u sadržaju), ne po medijateci — razlika je 66 vs 6.638; (2) **deljeni prilog** (u više galerija) mora imati neutralan opis — skripta **puca bez upisa** ako nije pokriven override mapom, umesto da tiho upiše naslov jednog proizvoda; (3) opis scene se piše tek **pošto je slika vizuelno pregledana**, sve ostalo je naslov proizvoda ili „— fotografija N". Ne dira postojeći alt. Prethodnik `job-alt-tekst-proizvodi.php` pokriva samo glavnu sliku |
